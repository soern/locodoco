...
void ldc_packet_handler ()
{
	...
	switch (LDC_TYPE_GET(rxbuf))
	{
		...
		case LDC_CMD_REBOOT:
			...
			WDTCSR  = (_BV(WDE) | _BV(WDP0)); /* 64ms */
			MCUSR |= _BV(PORF); /* run bootloader */
			sei();
			while(23)
				asm volatile ("nop");
		break;
		...
		{
			ldc_speed_t *s = (ldc_speed_t *) LDC_PAYLOAD(rxbuf);
			//motor_set_speed_cmd ((ldc_speed_t *) LDC_PAYLOAD(rxbuf));
			motor_set_direction (s->dir);
			motor_set_target_speed (s->speed);
		}
		break;
