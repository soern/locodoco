...
static char *str_search (char* in_h, const char* in_n)
{
...
static ssize_t parse_u8data (uint8_t *odata, char* in_str, size_t maxlen)
{
	ssize_t i;

	memset (odata, 0x00, maxlen);

	if (maxlen > strlen(in_str))
	...
	for (i=0; i<maxlen; i+=2)
	{
		...
		printf("chars: %c%c, val = %02x\r\n", in_str[i], in_str[i+1], val);
		odata[i/2] = val;
	}
...
// XXX XXX XXX BUGGY FUNC?!
static int parse_atoi (char* in_str)
{
	char *p = in_str;
...
int parse_qstr ()
{
	...
	t = str_search (q, "u8data=");
	if (t != NULL)
	{
		ssize_t len;
		uint8_t *p = txpacket + sizeof(ldc_header_t) + sizeof(rfm12_header_t) -1;
		len = parse_u8data(p, t+7, sizeof(txpacket) - (p - txpacket));

		txlen += len;
		printf ("u8data (%zu) = %02x %02x %02x %02x ...\n", len, p[0], p[1], p[2], p[3]);
	}
...
int main (int argc, char* argv[])
{
	...
	if (parse_qstr() >= 0)
	{
		...
		printf ("OK, cmd = %02X, dst = %04X %u<br />\r\n", lh->ltype, lh->addr, lh->addr);
